import requests
import re

# 1. 定义官方源地址
url = "https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/config/ACL4SSR_Online_Full_MultiMode.ini"

# 2. 下载文件
print("正在下载官方配置...")
r = requests.get(url)
if r.status_code != 200:
    print("下载失败")
    exit(1)
content = r.text

print("下载成功，开始修改配置...")

# ===============================================================
# 3. 定义你要添加的 3 个新策略组
#    格式说明：custom_proxy_group=组名`类型`筛选条件`测速地址`间隔,,容差
# ===============================================================

# 这里的逻辑是：先定义字符串，后面插入到文件中
new_groups_def = """
; === 自定义新增策略组 Start ===
custom_proxy_group=自定义香港高级BGP负载均衡`load-balance`(香港 高级中继)`http://www.gstatic.com/generate_204`300,,50
custom_proxy_group=自定义香港IEPL负载均衡`load-balance`(香港 IEPL)`http://www.gstatic.com/generate_204`300,,50
custom_proxy_group=自定义日常工作`url-test`[]自定义香港IEPL负载均衡`[]自定义香港高级BGP负载均衡`[]♻️ 自动选择`http://www.gstatic.com/generate_204`300,,50
; === 自定义新增策略组 End ===
"""

# ===============================================================
# 4. 执行自动化修改逻辑
# ===============================================================

# 【修改操作 A】插入新策略组定义
# 我们把新定义的组，插入到 ";设置分组标志位" 这一行的后面，这样它们就生效了
if ";设置分组标志位" in content:
    content = content.replace(";设置分组标志位", ";设置分组标志位\n" + new_groups_def)
else:
    # 如果找不到标志位，就加在 [custom] 后面
    content = content.replace("[custom]", "[custom]\n" + new_groups_def)
print("✅ 已创建 3 个自定义策略组")


# 【修改操作 B】修改 "🚀 节点选择"
# 目标：把 "自定义日常工作" 插在 "select`" 的后面，作为第一个选项
# 原文类似：custom_proxy_group=🚀 节点选择`select`[]♻️ 自动选择`...
# 我们查找：custom_proxy_group=🚀 节点选择`select`
# 替换为：custom_proxy_group=🚀 节点选择`select`[]自定义日常工作`
content = content.replace(
    "custom_proxy_group=🚀 节点选择`select`", 
    "custom_proxy_group=🚀 节点选择`select`[]自定义日常工作`"
)
print("✅ 已将 [自定义日常工作] 加入到节点选择首位")


# 【修改操作 C】重写 "💬 Ai平台"
# 目标：只要包含 GPT、Gemini、Ai 的节点
# 我们使用正则表达式，找到以 "custom_proxy_group=💬 Ai平台" 开头的整行，直接替换掉
# 这里的 (GPT|Gemini|Ai) 就是由于你需要匹配这三个关键字
new_ai_group = "custom_proxy_group=💬 Ai平台`select`(GPT|Gemini|Ai)"

# re.sub(正则表达式, 替换内容, 原文)
# ^custom_proxy_group=💬 Ai平台.* 代表从这句开头匹配到行尾
content = re.sub(
    r"^custom_proxy_group=💬 Ai平台.*", 
    new_ai_group, 
    content, 
    flags=re.MULTILINE
)
print("✅ 已修改 [💬 Ai平台] 为仅筛选 GPT/Gemini/Ai")


# ===============================================================
# 5. 保存为新文件
# ===============================================================
filename = "ACL4SSR_Custom.ini"
with open(filename, "w", encoding="utf-8") as f:
    f.write(content)

print(f"🎉 成功更新并保存为 {filename}")
